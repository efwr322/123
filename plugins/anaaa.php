...
class anaaa extends phplistPlugin {
  const CODE_DIR = '/123/';
  public $name = "test";
  public $coderoot = '';
  public $version = "0.1";
  ...
  function __construct() {
    ...
    $this->numcriterias = 2;
    $this->numcriterias = getConfig('anaaa_numcriterias');
    $GLOBALS['anaaa_criteriacache'] = array();
  }
...
#      $criteria_content .= '</table>';
  ...
  function canSend ($messagedata, $userdata) {
    ...
    if (!isset($GLOBALS['anaaa_criteriacache'][$messagedata['id']])) {
      $GLOBALS['anaaa_criteriacache'][$messagedata['id']] = array();
      $sql = $this->subscriberSelection($messagedata);
      if (!empty($sql)) {
        $req = Sql_Query($sql);
        while ($row = Sql_Fetch_Row($req)) {
          $GLOBALS['anaaa_criteriacache'][$messagedata['id']][] = $row[0];
        }
      }
    }
    //ob_end_clean();
    //var_dump($GLOBALS['anaaa_criteriacache'][$messagedata['id']]);
    $cansend = sizeof($GLOBALS['anaaa_criteriacache'][$messagedata['id']]) && in_array($userdata['id'],$GLOBALS['anaaa_criteriacache'][$messagedata['id']]);
    if (VERBOSE) {
      if ($cansend) {
        cl_output('anaaa - CAN SEND '.$userdata['id']);
      } else {
        cl_output('anaaa - CAN NOT SEND '.$userdata['id']);
      }
    }
    return $cansend;
  }
